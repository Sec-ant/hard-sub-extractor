...
from .utility import get_image_file_list, check_and_read_gif, get_logger, parse_args, init_args
...
class TextSystem(object):
    ...
    def __call__(self, img, det=True, cls=False, rec=True, return_cls=False):
        ...
        if det:
            ...
            dt_boxes = [box.astype('float').tolist() for box in sorted_dt_boxes]
            det_run_once = True

        img_cropped = False
        cls_run_once = False
        if cls:
            if det_run_once and not img_cropped:
                img_crop_list = []
                for bno in range(len(sorted_dt_boxes)):
                    tmp_box = copy.deepcopy(sorted_dt_boxes[bno])
                    img_crop = get_rotate_crop_image(img, tmp_box)
                    img_crop_list.append(img_crop)
                img_cropped = True
            else:
